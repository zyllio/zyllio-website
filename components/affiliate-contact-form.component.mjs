...
class ZyllioAffiliateContactForm extends HTMLElement {
  constructor() {
    super()
    ...
    this.boundDelegated = (event) => {
      try {
        const selector = this.triggerSelector
        if (!selector) return
        const el = event.target && event.target.closest && event.target.closest(selector)
        if (el) {
          event.preventDefault()
          this.open()
        }
  ...
  connectedCallback() {
    ...
    if (triggerSelector) {
      document.addEventListener('click', this.boundDelegated)
    }
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        const submittedPayload = await submitContactPayload(payload, 'affiliation', this.locale)
        form.reset()
        messageInput.value = this.initialMessage
        this.setStatus()
        dialog.close()
        ...
        this.dispatchEvent(new CustomEvent('zyllio-affiliate-form-submit', {
  ...
  disconnectedCallback() {
    ...
    if (this.triggerSelector) {
      document.removeEventListener('click', this.boundDelegated)
    }
    const closeButton = this.shadowRoot.querySelector('.close')
...
customElements.define('zyllio-affiliate-contact-form', ZyllioAffiliateContactForm)